...
vector<vector<int>> vcids;                                      // to store VCIDS for connections
...
{                                                               // default values to the structures 
    ...
    vcids=vector<vector<int>> (nodes,vector<int> (nodes,1000));
...
int writeForwardTable(string FW_file,int src,int dest,vector<int>& p,vector<int>& vcidList)
{                                                                      // function to write Forward Table 
    ...
    fw_file<<right<<setfill(' ')<<setw(10)<<vcids[src][p[1]]<<"\n";
    vcidList.push_back(vcids[src][p[1]]);
    vcids[src][p[1]]+=50;
    ...
    for(auto i=1;i<p.size()-1;i++)
    {
        ...
        fw_file<<right<<setfill(' ')<<setw(10)<<vcids[p[i]][p[i-1]];        // VCID
        fw_file<<right<<setfill(' ')<<setw(10)<<p[i+1];                     // ID of outgoing port
        fw_file<<right<<setfill(' ')<<setw(10)<<vcids[p[i]][p[i+1]]<<"\n";  // VCID
        vcidList.push_back(vcids[p[i]][p[i+1]]);
        vcids[p[i]][p[i-1]]+=50;
        vcids[p[i]][p[i+1]]+=50;
        temp_s=p[i];
    }
    ...
    fw_file<<right<<setfill(' ')<<setw(10)<<vcids[dest][temp_s];
    fw_file<<right<<setfill(' ')<<setw(10)<<"--";
    fw_file<<right<<setfill(' ')<<setw(10)<<"--"<<"\n";
    vcids[dest][temp_s]+=50;
    fw_file.close();
    return 0;
}

void writeConnTable(int connID, int src, int dest, vector<int> &p,vector<int> &vcidList)
{
    fstream cn_file;                                                   // information in file
    cn_file.open("ConnTableFile.txt", ios::app | ios::out );                       // to open file in write and append mode
    if(!cn_file)
    {
        cout<<"\n Unable to write in Connection table file"<<endl;
        return ;
    }

    int pCost=getDelay(src,p);
    cn_file<<right<<setfill(' ')<<setw(10)<<connID;
    cn_file<<right<<setfill(' ')<<setw(10)<<src;
    cn_file<<right<<setfill(' ')<<setw(10)<<dest;
    cn_file<<right<<setfill(' ')<<setw(10);
    for(auto it:p)
        cn_file<<it<<"->";
    cn_file<<right<<setfill(' ')<<setw(10);
    for(auto it:vcidList)
        cn_file<<it<<", ";
    cn_file<<right<<setfill(' ')<<setw(20)<<pCost<<"\n";
}

int setConnection(string FW_file,int src,int dest,float equi_cap, int connID)                           // function to check if connection request can 
{                                                           // be accepted and to keep track of accepted 
    vector<int> p, vcidList;
    ...
    {                                                                 // every intermediate link of path then
        ...
        {
            writeForwardTable(FW_file,src,dest,p,vcidList);                    // capacity matrix and writing into file
            writeConnTable(connID,src,dest,p,vcidList);
        }
        connAccepted++;
        VCID+=dist*10;
    }
    ...
    {                                                                 // it checks for second shortest path
        ...
        if(set)
        {
            ...
            {
                writeForwardTable(FW_file,src,dest,p1,vcidList);
                writeConnTable(connID,src,dest,p1,vcidList);
            }
            connAccepted++;
            VCID+=dist*10;
        }
...
int readConnFile(string connFile, string FW_file)                              // function to read Connection requests
{
    ...
    fstream fw_file,cn_file;                                        // connections, link capacity, VCID, etc
    fw_file.open(FW_file, ios::trunc | ios::out);
    cn_file.open("ConnTableFile.txt", ios::trunc | ios::out);
    ...
    if(p==0)
        ...
        {                                                      // to check proper file format 
            ...
            setConnection(FW_file,src,dest,equi_cap,cnt++);
            //cnt++;
        }
    else
    {
        while(fin>>src && fin>>dest && fin>>min_c && fin>>avg_c && fin>>max_c)
        {
            ...
            setConnection(FW_file,src,dest,max_c,cnt++);
            //cnt++;
        }
    }
    connReq=cnt;
